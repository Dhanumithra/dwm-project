...
// Initials-based avatar for each department card (no emojis)
function DeptAvatar({ name }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: "#fff", fontWeight: 800, fontSize: 14, letterSpacing: 0.5 }}>{initials}</span>
    </div>
  );
}
...
export default function Departments() {
  ...
  const addDept = () => {
    ...
    (async () => {
      try {
        const created = await deptApi.create({ name: newDept.name, description: newDept.description, headCount: 0 });
        setDepartments((prev) => [...prev, created]);
        setNewDept({ name: "", description: "" });
        setShowAdd(false);
        showMsg("success", "Department added successfully.");
      } catch (err) {
        showMsg("warning", "Failed to add department.");
      }
    })();
  };
  ...
  const saveEdit = () => {
    (async () => {
      try {
        const updated = await deptApi.update(editDept.id, editDept);
        setDepartments((prev) => prev.map((d) => d.id === updated.id ? updated : d));
        setEditDept(null);
        showMsg("success", "Department updated.");
      } catch (err) {
        showMsg("warning", "Failed to update department.");
      }
    })();
  };
      ...
      <div className="container-fluid">
        ...
        {showAdd && (
          <div className="card mb-4" style={{ padding: 20 }}>
            <h6 style={{ fontWeight: 700, marginBottom: 14, fontSize: 14, color: "#0f172a", textTransform: "none", letterSpacing: 0 }}>
              New Department
            </h6>
            <div className="row g-2">
              <div className="col-md-4">
                <label style={{ fontWeight: 600, fontSize: 13, color: "#475569" }}>Department Name</label>
              ...
              <div className="col-md-6">
                <label style={{ fontWeight: 600, fontSize: 13, color: "#475569" }}>Description (optional)</label>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{
                            ...
                            <span style={{ color: "#fff", fontWeight: 800, fontSize: 10 }}>
                              {dept.name.split(/[\s\-\/]+/).slice(0, 2).map((w) => w[0]).join("").toUpperCase()}
                            </span>
                          </div>
                          <strong style={{ fontSize: 13.5 }}>{dept.name}</strong>
                        </div>
                      </td>
